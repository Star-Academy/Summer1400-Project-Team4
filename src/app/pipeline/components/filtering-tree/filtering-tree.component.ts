...
export class FilteringTreeComponent implements OnInit {
    ...
    constructor(
        ...
        public dialogRef: MatDialogRef<FilteringTreeComponent>,
    ...
    ngOnInit(): void {
                ...
                this.ogma.tools.connectNodes.enable({
                    ...
                    condition: (source: any, target: any) => {
                        ...
                        for (let i = 0; i < edgeList.size; i++) {
                            ...
                            if (
                                edgeData.source == source.getId() &&
                                edgeData.target == target.getId()
                            )
                                return false;
                        }
                        return (
                            source.getData().type != 'child' &&
        ...
        this.ogma.events.onDragEnd(() => {
            if (this.ogma.algorithms.detectCycle())
                this.ogma.removeEdge(
                    this.ogma
                        .getEdges()
                        .get(this.ogma.getEdges().size - 1)
                        .getId()
                );
        });
    }
    ...
    handleSavingGraph() {
        ...
        if (root.size !== 1) {
            ...
            if (!this.errorGraph) {
                ...
                alert(
                    'خطا در ذخیره سازی : گره های غیر شرطی حداقل 2 فرزند باید داشته باشند'
                );
    ...
    applyingDFS(node: nodeData): FilterData | void {
        ...
        if (node.getData('type') == 'child') {
            info = {
                ...info,
                ...{
                    _field: node.getData('_field'),
                    _value: node.getData('_value'),
                },
            };
        } else {
            ...
            nodeObjs.forEach((n: any, i: number): void => {
                ...
                if (childStatement) _statement.push(childStatement);
                else {
